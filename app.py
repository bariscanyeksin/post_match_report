...
#### DEĞİŞTİRİLMESİ GEREKEN VERİLER ####

whoscored_match_id = 1842978
fotmob_match_id = 4549687
fotmob_league_id = 71
...
def getFotmobData():
    ...
    fotmob_match_response = requests.get(fotmob_match_url)
    fotmobData = fotmob_match_response.json()
    return fotmobData

fotmobData = getFotmobData()
...
def list_teams_from_df(players_df, teams_dict):
    ...
    print(f"Ev Sahibi Takım: {home_team_name} (ID: {home_team_id})")
    print(f"Deplasman Takımı: {away_team_name} (ID: {away_team_id})")
...
def match_details(ax):
    ...
    ax.text(x_center, 0.15, "Pas", size=10, ha="center", fontproperties=prop, bbox=back_box, color='black')
        ...
        for shot in home_shots:
            if shot['isOwnGoal'] != True:
                if shot['eventType'] == 'Goal':  # Gol olan şut
                    ...
                    pitch.scatter(shot['x'], shot['y'], ax=ax, c='None', s=round(shot['expectedGoals'], 2)*300, edgecolors=goal_color, marker='football', alpha=0.8)
                    ...
                    pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*300, edgecolors='white', marker='D', alpha=0.8, lw=0.7, hatch='|||||')
                
                elif shot['eventType'] == 'AttemptSaved' and shot['isBlocked'] == False:  # İsabetli Şut | Kurtarılan şut
                    shot_color = saved_color
                    #pitch.lines(shot['x'], shot['y'], shot['blockedX'], shot['blockedY'], ax=ax, color=to_rgba(shot_color, alpha=0.5), lw=1)
                    pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*300, edgecolors='white', marker='o', alpha=0.8, lw=1, hatch='/////')
                    ...
                    pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*300, edgecolors='black', marker='x', alpha=0.8, lw=1)
                
                elif shot['eventType'] == 'Post':  # Direkten dönen şut
                    shot_color = post_color
                    #pitch.lines(shot['x'], shot['y'], 105, shot['goalCrossedY'], ax=ax, color=to_rgba(shot_color, alpha=0.25), lw=1)
                    pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*300, edgecolors='black', marker='+', alpha=0.8, lw=1, hatch='|')
                
                else:  # Bloklanan diğer şutlar
                    shot_color = blocked_color
                    #pitch.lines(shot['x'], shot['y'], shot['blockedX'], shot['blockedY'], ax=ax, color=to_rgba(shot_color, alpha=0.5), lw=1)
                    pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*300, edgecolors='black', marker='o', alpha=0.8, lw=1, hatch='/')
...
def momentum(ax):
    ...
    redcard_events = [event for event in card_events if event['card'] == 'Red']
    ...
    start_minute = 0
    ht_minute = 45  # HT için dakika
    ft_minute = 90  # FT için dakika
...
def pass_network_visualization(ax, passes_between_df, average_locs_and_count_df, col, teamName, flipped=False): 
    ...
    passes_between_df = passes_between_df[passes_between_df['isFirstEleven'] == True]
    ...
    pitch = Pitch(pitch_type='uefa', corner_arcs=True, pitch_color=pitch_color, line_color=line_color, linewidth=2, goal_type='box')
    pitch.draw(ax=ax)
    # Initial setup
    ax.set_xlim(-0.5, 105.5)
    ...
    if teamName == away_team_name:
        ...
        def_line_horizontal_alignment = 'right'
        def_line_vertical_alignment = 'top'
        def_line_text_y = 1.25
    else:
        text_x = 0
        text_y = -4
        annotate_padding = 0.4
        def_line_horizontal_alignment = 'left'
        def_line_vertical_alignment = 'bottom'
        def_line_text_y = 1
    ...
    for index, row in average_locs_and_count_df.iterrows():
      if row['isFirstEleven'] == True:
        pass_nodes = pitch.scatter(row['pass_avg_x'], row['pass_avg_y'], s=600, marker='o', color=pitch_color, edgecolor=line_color, linewidth=2, alpha=1, ax=ax)

    # Plotting the shirt no. of each player
    for index, row in average_locs_and_count_df.iterrows():
        player_initials = row["shirtNo"]
        if row['isFirstEleven'] == True:
            pitch.annotate(player_initials, xy=(row.pass_avg_x, row.pass_avg_y-annotate_padding), c=col, ha='center', va='center', size=14, ax=ax, fontproperties=prop)
    ...
    distance_between_lines = round(fwd_line_h - def_line_h, 2)
    
    ax.text(def_line_h, def_line_text_y, f'{distance_between_lines}m', 
        horizontalalignment=def_line_horizontal_alignment, verticalalignment=def_line_vertical_alignment, color=col, fontsize=8, fontproperties=prop)
...
def key_passes(ax):
    ...
    home_key_passes = df_key_passes[df_key_passes['teamId'] == home_team_id]
    away_key_passes = df_key_passes[df_key_passes['teamId'] == away_team_id]
...
def box_entry(ax):
    ...
    pitch = Pitch(pitch_type='uefa', line_color=line_color, corner_arcs=True, line_zorder=2, pitch_color=pitch_color, linewidth=2)
    pitch.draw(ax=ax)
    ax.set_xlim(-0.5, 105.5)
    ax.set_ylim(-0.5, 68.5)
    ...
    pitch.lines(x_center/8+4, 5, x_center/8+16, 5, lw=3.5, transparent=True, comet=True, color=color, ax=ax)
    pitch.scatter(x_center/8+16, 5, s=35, edgecolor=color, linewidth=1, color=pitch_color, zorder=2, ax=ax)
...
def defensive_block(ax, average_locs_and_count_df, team_name, col):
    ...
    pitch = Pitch(pitch_type='uefa', pitch_color=pitch_color, line_color=line_color, linewidth=2, line_zorder=2, corner_arcs=True)
    pitch.draw(ax=ax)
    ax.set_facecolor(pitch_color)
    ax.set_xlim(-0.5, 105.5)
    ax.set_ylim(-0.5, 68.5)
    ...
    if team_name == home_team_name:
        ...
        ax.text(0,-10,'-------', fontsize=12, fontproperties=prop, color=pitch_color, ha='left', va='center')
    else:
...
def plot_congestion(ax):
    pitch = Pitch(pitch_type='uefa', pitch_color=pitch_color, line_color=line_color, goal_type='box', corner_arcs=True, line_zorder=6)
    pitch.draw(ax=ax)
    ax.set_xlim(-0.5,105.5)
    ax.set_ylim(-0.5,68.5)
    plt.axis('off')
    ...
    df1 = df[(df['teamName']==home_team_name) & (df['isTouch']==1) & (~df['qualifiers'].str.contains('CornerTaken|Freekick|ThrowIn'))]
    df2 = df[(df['teamName']==away_team_name) & (df['isTouch']==1) & (~df['qualifiers'].str.contains('CornerTaken|Freekick|ThrowIn'))]
    df2['x'] = 105-df2['x']
    df2['y'] =  68-df2['y']
...
def generate_and_save_figure():
    ...
    fig.text(0.2, 0.915, matchDetailString, fontsize=20, ha='left', va='center', fontproperties=prop, color=line_color)
    ...
    replace_result = result_string.replace(' ', '_')
    replace_formatted_date = formatted_date.replace('.', '_')
    
    # Klasör yolunu belirt
    folder_path = 'görseller'
    ...
    # Dosya kaydetme
    plt.savefig(f'{folder_path}\\{replace_result}_{replace_formatted_date}.png', bbox_inches='tight', dpi=300)
    plt.close() 
